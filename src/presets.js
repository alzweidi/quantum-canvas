...
function calculateAdaptiveSlitGeometry (width, height) {
  ...
  if (totalSlitSpace > maxUsableHeight) {
    ...
    slitHeight = Math.max(
      minSlitHeight,
      Math.floor(baseSlitHeight * scaleFactor)
    )
  }
...
function validatePresetCompatibility (presetName, width, height) {
  ...
  if (width < minGridSize || height < minGridSize) {
    compatible = false
    warnings.push(
      `Grid too small (${width}x${height}). Minimum size: ${minGridSize}x${minGridSize}`
    )
  }
  ...
  if (presetName === 'DOUBLE_SLIT') {
    ...
    if (geom.slitHeight < 3 || geom.slitGap < 6) {
      warnings.push(
        'Grid size may produce suboptimal double slit visualization'
      )
    }
  }
  ...
  if (presetName === 'TUNNELING') {
    ...
    if (geom.barrierThickness < 3) {
      warnings.push(
        'Grid size may produce thin tunneling barrier - consider larger grid'
      )
    }
  }
...
export const PRESETS = {
  DOUBLE_SLIT: {
    ...
    draw: (potential, width, height, barrierPhaseKick) => {
      ...
      const validation = validatePresetCompatibility(
        'DOUBLE_SLIT',
        width,
        height
      )
      if (!validation.compatible) {
        console.error(
          'DOUBLE_SLIT preset incompatible with grid size:',
          validation.warnings
        )
        return
      }
      ...
      validation.warnings.forEach((warning) =>
        console.warn('DOUBLE_SLIT:', warning)
      )
  ...
  TUNNELING: {
    ...
    draw: (potential, width, height, barrierPhaseKick) => {
      ...
      if (!validation.compatible) {
        console.error(
          'TUNNELING preset incompatible with grid size:',
          validation.warnings
        )
        return
      }
      ...
      // log warnings if any
      validation.warnings.forEach((warning) =>
        console.warn('TUNNELING:', warning)
      )
      ...
      // draw barrier with bounds-safe iteration
      const xStart =
        geom.barrierCenterX - Math.floor(geom.barrierThickness / 2)
