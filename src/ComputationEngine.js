...
export class ComputationEngine {
    ...
    _validateGridDimensions(gridSize) {
        if (!gridSize || typeof gridSize.width !== 'number' || typeof gridSize.height !== 'number') {
            throw new Error('Invalid grid dimensions: must have numeric width and height');
        }
        
        if (gridSize.width <= 0 || gridSize.height <= 0) {
            throw new Error(`Invalid grid dimensions: width=${gridSize.width}, height=${gridSize.height} - must be positive`);
        }
        
        if (!Number.isInteger(gridSize.width) || !Number.isInteger(gridSize.height)) {
            throw new Error(`Invalid grid dimensions: width=${gridSize.width}, height=${gridSize.height} - must be integers`);
        }
        
        // check if dimensions are powers of 2 (optimal for FFT)
        const isPowerOf2 = (n) => n > 0 && (n & (n - 1)) === 0;
        if (!isPowerOf2(gridSize.width) || !isPowerOf2(gridSize.height)) {
            console.warn(`[WARNING] Grid dimensions ${gridSize.width}x${gridSize.height} are not powers of 2. FFT performance may be suboptimal.`);
        }
    }
...
for (let i = 0; i < this.buffer1.length; i += 2) {
    ...
    testRoundTripAccuracy(width, height) {
        ...
        console.log(`Impulse injected at (${centerX}, ${centerY}), idx=${centerIdx}`);
        console.log(`Original: real=${testGrid[centerIdx]}, imag=${testGrid[centerIdx + 1]}`);
        ...
        console.log(`Round-trip result: real=${buffer2[centerIdx]}, imag=${buffer2[centerIdx + 1]}`);
