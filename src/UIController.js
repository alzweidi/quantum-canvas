...
export class UIController {
  ...
  _setupSliders () {
    ...
    this._setupSlider(
      'px-slider',
      'px-value',
      (val) => (this.state.params.px = parseFloat(val))
    )
    this._setupSlider(
      'py-slider',
      'py-value',
      (val) => (this.state.params.py = parseFloat(val))
    )
    this._setupSlider(
      'sigma-slider',
      'sigma-value',
      (val) => (this.state.params.sigma = parseFloat(val))
    )
  }
  ...
  _handleMouseUp (event) {
    ...
    if (this.mouseMode === 'nudge') {
      ...
      if (pxValue) {
        ...
        if (DEBUG) {
          console.warn(
            'Warning: px-value element not found for momentum update'
          )
        }
      }
      ...
      if (pyValue) {
        ...
        if (DEBUG) {
          console.warn(
            'Warning: py-value element not found for momentum update'
          )
        }
      }
    }
    ...
    paramsToSync.forEach((param) => {
      ...
      if (slider && valueSpan) {
        ...
        if (DEBUG) {
          if (!slider) {
            console.warn(
              `Warning: ${sliderId} element not found during UI sync`
            )
          }
          if (!valueSpan) {
            console.warn(
              `Warning: ${valueId} element not found during UI sync`
            )
          }
        }
      }
