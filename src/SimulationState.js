...
export class SimulationState {
  ...
  _createReflectiveBoundary () {
    ...
    if (
      this.params.boundaryMode === 'reflective' ||
   ...
   */
  _getPxMax () {
    return (Math.PI / this._getDx()) * C.HBAR
  } // Nyquist px
   ...
   */
  _getPyMax () {
    return (Math.PI / this._getDy()) * C.HBAR
  } // Nyquist py
  ...
  _clampMomentumToNyquist (margin = 0.9) {
    ...
    const oldPx = this.params.px
    const oldPy = this.params.py
        ...
        for (let j = 0; j < width; j++) {
          ...
          if (minDist < boundaryWidth) {
            ...
            const dampingFactor = Math.exp(
              -dampingRate * this.params.dt * factor
            )
