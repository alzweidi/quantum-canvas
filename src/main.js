...
console.log(
  "Wave function normalization check (should ≈ 1.0):",
  psiMagnitudeSum,
);
...
function checkWaveFunction(state, label) {
  ...
  for (let i = 0; i < state.psi.length; i++) {
    ...
    if (i % 2 === 0) {
      // Summing magnitude squared: real^2 + imag^2
      sumOfSquares +=
        state.psi[i] * state.psi[i] + state.psi[i + 1] * state.psi[i + 1];
    }
  }
  if (hasNaN) {
    ...
    console.log(
      `CHECK at ${label}: Normalization = ${sumOfSquares.toFixed(6)}`,
    );
  }
}
...
function gameLoop() {
  if (frameCount < 5) {
    // Only log the first 5 frames to avoid spam
...
console.log(
  "Interactive quantum simulator ready - click and drag to draw barriers!",
);
gameLoop();
