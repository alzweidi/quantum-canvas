...
console.log(
  `[DPR FIX] Canvas sizing - DPR: ${devicePixelRatio}, CSS: ${C.GRID_SIZE}x${C.GRID_SIZE}, Backing store: ${backingStoreWidth}x${backingStoreHeight}`
)
...
function handleVisibilityChange () {
  ...
  if (isTabVisible && !wasVisible) {
    ...
    // tab became hidden
    console.log(
      '[VISIBILITY] Tab became hidden - pausing animation to save resources'
    )
    if (animationFrameId) {
      cancelAnimationFrame(animationFrameId)
      animationFrameId = null
...
function handleDPRChange () {
  ...
  if (newDPR !== currentDPR) {
    console.log(
      `[DPR CHANGE] Device pixel ratio changed from ${currentDPR} to ${newDPR} - updating canvas`
    )
    ...
    if (gl) {
      const maxTextureSize = gl.getParameter(gl.MAX_TEXTURE_SIZE)
      if (
        newBackingStoreWidth > maxTextureSize ||
        newBackingStoreHeight > maxTextureSize
      ) {
        console.warn(
          `[DPR CHANGE] New backing store ${newBackingStoreWidth}x${newBackingStoreHeight} exceeds max texture size ${maxTextureSize}, keeping current size`
        )
    ...
    try {
      ...
      console.log(
        `[DPR CHANGE] Successfully updated renderer for ${newBackingStoreWidth}x${newBackingStoreHeight} backing store`
      )
    } catch (error) {
